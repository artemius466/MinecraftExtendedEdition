...
local function get_object_list_by_page(page)
    ...
    local min_page = page*13
    local max_page = min_page+13
    for i, v in pairs(objects) do
        if count >= min_page then
            if count <= max_page then
...
local max_page = get_last_page()
local buttons_on_page = 13

local currentPageBtns = {}
...
local function select(down)
    if down == true then
        if not (selectedButton == buttons_on_page) then
        ...
        if not (selectedButton == 1) then
            ...
            selectedButton = buttons_on_page
        end
        cooldown = get_global_timer() + 4
    end
...
local function on_hud_render()
    if menuOpened == true then
        ...
        local i = 1
        while i <= buttons_on_page do
            if selectedButton == i then
                djui_hud_button_render((width/2)-(buttonWidth/2)+(screenWidth/2)-(width/2), ((buttonHeight+(buttonHeight/5))*i)+height/20, buttonWidth, buttonHeight, 2, true, currentPageBtns[i])
            else
                djui_hud_button_render((width/2)-(buttonWidth/2)+(screenWidth/2)-(width/2), ((buttonHeight+(buttonHeight/5))*i)+height/20, buttonWidth, buttonHeight, 2, false, currentPageBtns[i])
            end
            i = i + 1
        end
